...
| **React** | **Staff/Head Viewer Landing Page:** Top nav, category card grid (no TRASH), folder grid/list, document list, document view panel, AttachmentPreview (paginated), Export PDF button. **Secretary → Documents Page:** Same layout + all categories including TRASH (Secretary view). Secretary action kebab menu on document cards. **Secretary → Add Document Page:** Path-based file picker, thumbnail strip, metadata form, Save button. Confidential status auto-enables Hidden. **Secretary → Dashboard:** Stats widgets, recent documents list. |
