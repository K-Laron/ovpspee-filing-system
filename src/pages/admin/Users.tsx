...
export const Users = () => {
  ...
  const reload = async () => {
    ...
    try {
      setUsers(
        await invoke<UserItem[]>('list_users', { sessionId, search: search.trim() || null }),
      );
    } catch (err) {
    ...
    <div className="mx-auto max-w-6xl space-y-5">
      ...
      >
        <div className="relative flex-1">
          ...
          <input
            className="focus-ring h-10 w-full rounded border border-border bg-white pl-9 pr-3 text-sm"
        ...
        <button
          className="focus-ring rounded bg-secondary px-4 text-sm font-semibold text-white"
          type="submit"
        >
          Search
        </button>
      ...
      <section className="grid gap-5 lg:grid-cols-[1fr_22rem]">
            ...
            <tbody className="divide-y divide-border">
              ...
              {!loading && users.length === 0 && (
                <tr>
                  <td className="px-4 py-6 text-center text-muted" colSpan={5}>
                    {search.trim()
                      ? 'No users match the current search. Clear the search or try another name, username, or email.'
                      : 'No users yet. Use the form on the right to create an Admin or Secretary account.'}
                  </td>
                </tr>
              )}
              {!loading &&
                  ...
                  <tr key={user.user_id}>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-2">
                        <IconButton label="Edit user" onClick={() => editUser(user)}>
                          <Edit2 size={15} />
                        </IconButton>
                        <IconButton
        ...
        <div className="space-y-4">
          ...
          >
            ...
            <button
              className="focus-ring inline-flex h-10 w-full items-center justify-center gap-2 rounded bg-primary px-3 text-sm font-semibold text-white hover:bg-secondary disabled:opacity-60"
              disabled={saving}
              type="submit"
            >
              <Plus size={16} />
          ...
          {resetTarget && (
            ...
            >
              <FormTitle
                editing
                label={`Reset ${resetTarget.username}`}
                onCancel={() => {
                  setResetTarget(null);
                  setResetPassword('');
                  setResetConfirmPassword('');
